...
describe('runTuiExec', () => {
  ...
  it('fails when Runtime marks the Review result invalid', async () => {
    ...
    expect(stderr).toHaveBeenCalledWith(expect.stringContaining('bari exec failed'));
  });
  ...
  it('fails authentication before creating a Session or sending a message', async () => {
    ...
    expect(stderr).toHaveBeenCalledWith(
      'bari exec failed: Sign in to MiniMax to use Agent features. Run `bari login`, then retry.\n',
    );
  });
