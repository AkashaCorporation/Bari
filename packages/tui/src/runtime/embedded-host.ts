...
export async function createEmbeddedRuntimeHost(
  ...
  if (process.env.MAVIS_LOCAL_RUNTIME_V2_FORCE_LEGACY === '1') {
    throw new Error(
      'Bari embedded Runtime requires the local-runtime-v2 front door; legacy fallback is disabled.',
    );
  }
  ...
  try {
    await host.ready;
    if (!host.cliService) {
      throw new Error('Bari embedded Runtime does not expose CliService.');
    }
