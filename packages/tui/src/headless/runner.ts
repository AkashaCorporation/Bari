...
export async function runTuiExec(
  ...
  try {
    ...
    if (exitCode !== MINIMAX_CODE_EXEC_EXIT_CODES.brokenPipe) {
      diagnostics.push(
        `bari exec failed: ${formatTuiExecFailure(normalized)}\n`,
      );
    }
  } finally {
    ...
    try {
      ...
      shutdownFailure = formatTuiActionFailure(error, {
        summary: "bari exec shutdown failed.",
    ...
    if (terminal) {
      ...
      try {
        ...
        if (terminal.result.status !== "succeeded") {
          diagnostics.push(
            `bari exec ${terminal.result.status}: ${formatTuiExecResultFailure(terminal.result)}\n`,
          );
        }
      } catch (error) {
        const normalized = normalizeRunError(error);
