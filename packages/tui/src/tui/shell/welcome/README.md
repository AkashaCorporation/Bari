...
The hero always uses the same ANSI Shadow glyphs: the six-line `BARI` at 27 columns or more and the same `B` glyph at 8–26 columns. Do not invent a separate pixel font or stack `BA` / `RI` on narrow screens. Compact Welcome must still render the hero and show the product name only once in the frame header.
