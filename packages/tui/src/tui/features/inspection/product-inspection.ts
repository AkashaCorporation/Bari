...
export function createTuiAccountStatusInspection(
  ...
  return {
    title: `Bari status${options.version ? ` · v${safeInline(options.version)}` : ''}`,
        ...
        ? [
            {
              ...
              rows: [
                {
                  label: 'Current run',
                  value: 'Bari may use built-in defaults until the config is fixed and restarted.',
...
function configurationNextStep(diagnostics: TuiRuntimeDiagnostics): string {
  ...
  if (diagnostics.warnings.some((issue) => issue.includes('does not exist'))) {
    return `Create or restore ${path}, restart Bari, then run /doctor again.`;
  }
  if (diagnostics.warnings.some((issue) => issue.includes('cannot be read'))) {
    return `Check access to ${path}, restart Bari, then run /doctor again.`;
  }
  if (diagnostics.warnings.some((issue) => issue.includes('defaultModel'))) {
    return `Set defaultModel in ${path} to an available provider/model, restart Bari, then run /doctor again.`;
  }
  return `Fix ${path}, restart Bari, then run /doctor again.`;
}
