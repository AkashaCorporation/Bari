...
export class TuiUpdateFlow {
  ...
  async show(): Promise<void> {
    ...
    if (this.updateTask) {
      this.options.append(
        'An Bari update is already running. Please wait for it to finish.',
        'warning',
      );
      return;
    }
    ...
    if (plan.kind === 'current') {
      this.options.append(
        plan.source === 'managed-installer'
          ? `Bari ${plan.currentVersion} is current on ${plan.channel}.`
          : `Bari ${plan.currentVersion} is current on @${plan.packageTag}.`,
      );
      return;
    }
    if (plan.kind === 'ahead') {
      this.options.append(
        `Bari ${plan.currentVersion} is newer than ${
  ...
  private startBackgroundUpdate(
    ...
    this.close(panel);
    this.options.append(
      `Bari update started in the background (${plan.currentVersion} → ${plan.latestVersion}).`,
    );
    const task = this.requireAdmission()
      ...
      .then(
        (outcome) => {
          if (!this.stopped) {
            this.options.append(`Bari update completed: ${outcome.message}`);
        ...
        (error: unknown) => {
          if (!this.stopped) {
            this.options.append(
              `Bari update failed: ${error instanceof Error ? error.message : String(error)}`,
              'error',
            );
          }
        },
  ...
  private async requireAdmission(): Promise<void> {
    ...
    if (admission && !admission.allowed) {
      throw new McodeUpdateAdmissionError(
        admission.reason ?? 'Finish active Bari work before updating.',
      );
    }
  }
}
