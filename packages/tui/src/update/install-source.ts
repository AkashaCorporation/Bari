...
export function resolveMcodeNpmDistribution(
  ...
  if (
    ...
    throw new Error(`Unsupported Bari npm registry: ${resolvedRegistry}`);
  }
  ...
  throw new Error(`Unsupported Bari npm package: ${String(packageName)}`);
}
...
export function buildMcodePackageManagerCommand(
  ...
  switch (source) {
    case 'npm-prefix': {
      if (!prefixInstall) throw new Error('Bari npm prefix ownership metadata is missing.');
      if (
        prefixInstall.packageName !== distribution.packageName ||
        new URL(prefixInstall.registry).href !== distribution.registry
      ) {
        throw new Error('Bari npm prefix ownership does not match the installed package.');
      }
      return {
...
export async function resolveLatestMcodeRegistryVersion(
  ...
  if (Array.isArray(version)) {
    if (version.length !== 1) {
      throw new Error('Bari registry returned an invalid latest version.');
    }
    version = version[0];
  }
  ...
  if (tag === 'latest' && parsed.includes('-')) {
    throw new Error('Bari latest must resolve to a stable semantic version.');
  }
  return parsed;
}
