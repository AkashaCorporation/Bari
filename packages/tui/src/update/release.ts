...
export function parseMcodeUpdateChannel(value: string): McodeUpdateChannel {
  if (!CHANNEL_PATTERN.test(value)) {
    throw new Error(`Unsupported Bari update channel: ${JSON.stringify(value)}`);
  }
...
export function parseMcodeVersion(value: string): string {
  if (!VERSION_PATTERN.test(value)) {
    throw new Error(`Invalid Bari version: ${JSON.stringify(value)}`);
  }
  return value;
}
...
export function parseAndVerifyMcodeReleaseManifest(
  ...
  try {
    ...
    throw new Error(`Bari release signature could not be verified: ${errorMessage(error)}`);
  }
  if (!authentic) throw new Error('Bari release signature verification failed.');

  let parsed: unknown;
  try {
    parsed = JSON.parse(Buffer.from(manifestBytes).toString('utf8'));
  } catch (error) {
    throw new Error(`Bari release manifest is not valid JSON: ${errorMessage(error)}`);
  }
  return validateManifest(parsed);
}

function validateManifest(value: unknown): McodeReleaseManifestV1 {
  if (!isRecord(value)) throw new Error('Bari release manifest must be an object.');
  if (value.schemaVersion !== 1) {
    throw new Error(`Unsupported Bari release schema: ${String(value.schemaVersion)}`);
  }
  ...
  if (!isRecord(value.targets)) throw new Error('Bari release targets are missing.');
...
function readBase64Signature(value: string): Buffer {
  ...
  if (!/^[A-Za-z0-9+/]+={0,2}$/u.test(normalized)) {
    throw new Error('Bari release signature is not valid base64.');
  }
  const signature = Buffer.from(normalized, 'base64');
  if (signature.length !== 64) throw new Error('Bari release signature has an invalid length.');
  return signature;
}
...
export function resolveMcodeReleaseTarget(
  ...
  throw new Error(`Unsupported Bari update host: ${platform}-${arch}`);
}
...
export function assertMcodeReleaseTargetAvailable(
  ...
  throw new Error(`Bari ${manifest.version} does not provide a release for ${target}.`);
}
