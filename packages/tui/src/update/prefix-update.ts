...
export function prepareMcodePrefixUpdateStaging(
  ...
  if (!existsSync(activeModulesRoot)) {
    throw new Error(`Active Bari modules directory is missing: ${activeModulesRoot}`);
  }
...
export function readMcodePrefixPackageMetadata(
  ...
  if (
    ...
    throw new Error(`Bari package metadata is invalid at ${manifestFile}.`);
  }
...
function validateMcodePrefixEntry(
  ...
  commandName: 'Bari' | 'mcode-tools' | 'SQLite',
...
export async function scheduleMcodePrefixUpdate(
  ...
  if (path.resolve(pendingFile) !== path.resolve(pending.activePrefix, PENDING_UPDATE_FILE)) {
    throw new Error(`Bari pending update file is outside its active prefix: ${pendingFile}`);
  }
...
export function inspectPendingMcodePrefixUpdate(
  ...
  if (!state) {
    throw new Error(`Bari pending update artifacts are incomplete at ${pendingFile}.`);
  }
  return {
    pendingFile,
    activation: pending,
...
async function waitForProcessExit(pid: number): Promise<void> {
  ...
  while (isProcessAlive(pid)) {
    if (Date.now() >= deadline) {
      throw new Error(`Bari restart waited too long for parent process ${pid} to exit.`);
    }
...
function acquireMcodePrefixActivatorLease(leaseFile: string, journalDigest: string): boolean {
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    if (existing && isProcessAlive(existing.pid)) {
      ...
      throw new Error('Another Bari prefix activator is running for a different journal.');
    }
    rmSync(leaseFile, { force: true });
  }
  throw new Error(`Bari prefix activator lease could not be acquired at ${leaseFile}.`);
}
...
function readPendingUpdateFile(file: string): {
  ...
  if (
    ...
    (value.packageName !== '@bari/code' &&
      value.packageName !== '@minimax/code' &&
      value.packageName !== '@minimax-ai/code') ||
    ...
    throw new Error(`Bari pending update metadata is invalid at ${file}.`);
  }
  ...
  if (
    ...
    throw new Error(`Bari pending update paths are invalid at ${file}.`);
  }
...
if (crypto.createHash('sha256').update(pendingContents).digest('hex') !== pendingDigest) throw new Error('Bari pending update journal changed after scheduling.');
...
// old prefix exits; a TUI-owned Runtime or another Bari session may legitimately take over 30s.
...
async function activate() {
  ...
  try {
    ...
    if (stagedManifest.name !== pending.packageName || stagedManifest.version !== pending.expectedVersion) throw new Error('Staged Bari package identity or version is invalid.');
    if (launchers.some((launcher) => !fs.existsSync(launcher.stagedPath))) throw new Error('Staged Bari launcher is missing.');
    ...
    if (manifest.name !== pending.packageName || manifest.version !== pending.expectedVersion) throw new Error('Activated Bari package identity or version is invalid.');
    await restart(restartArgs);
    writeStatus('completed');
    cleanup();
  } catch (error) {
