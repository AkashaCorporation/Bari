...
export class McodeUpdateService {
  ...
  async check(request: McodeUpdateRequest = {}): Promise<McodeUpdateCheckResult> {
    ...
    const timeout = setTimeout(() => {
      timedOut = true;
      controller.abort(new Error(`Bari update check timed out after ${timeoutMs}ms.`));
    }, timeoutMs);
    timeout.unref?.();
    try {
      ...
      if (!version && manifest.channel !== channel) {
        throw new Error(
          `Signed Bari release channel mismatch: expected ${channel}, got ${manifest.channel}.`,
        );
      }
      if (version && manifest.version !== version) {
        throw new Error(
          `Signed Bari release version mismatch: expected ${version}, got ${manifest.version}.`,
        );
      }
      ...
      if (timedOut) {
        throw new Error(`Bari update check timed out after ${timeoutMs}ms.`, { cause: error });
      }
      throw error;
    } finally {
  ...
  async apply(request: McodeUpdateRequest = {}): Promise<McodeUpdateApplyResult> {
    ...
    if (check.status === 'ahead' && !request.version) {
      throw new Error(
        `Installed Bari ${this.currentVersion} is newer than ${check.channel} ` +
    ...
    const artifactTimeout = setTimeout(() => {
      ...
      artifactController.abort(
        new Error(`Bari artifact download timed out after ${artifactTimeoutMs}ms.`),
      );
    }, artifactTimeoutMs);
    ...
    try {
      ...
      if (request.signal?.aborted) throw new McodeUpdateCancelledError();
      if (artifactTimedOut) {
        throw new Error(`Bari artifact download timed out after ${artifactTimeoutMs}ms.`, {
          cause: error,
        });
      }
      throw error;
    ...
    try {
      ...
      throw new Error(
        `Bari ${check.latestVersion} was not activated; the previous version is unchanged: ${errorMessage(error)}`,
        { cause: error },
      );
    }
  }
...
export function resolveMcodeInstallRoot(environment: NodeJS.ProcessEnv = process.env): string {
  ...
  if (process.platform === 'win32') {
    ...
    if (!localAppData)
      throw new Error('LOCALAPPDATA is required to resolve the Bari install root.');
...
export function readMcodeUpdateChannel(installRoot: string): McodeUpdateChannel {
  ...
  try {
    ...
    throw new Error(`Bari update channel config is invalid: ${errorMessage(error)}`);
  }
  if (
    ...
    throw new Error('Bari update channel config is missing channel.');
  }
...
function readInstalledPublicKey(installRoot: string, environment: NodeJS.ProcessEnv): string {
  ...
  if (!existsSync(metadataFile)) {
    throw new Error(
      'Bari update trust root is missing. Install with the official Shell/PowerShell installer first.',
    );
  }
  ...
  if (typeof metadata.publicKey !== 'string' || !metadata.publicKey.includes('BEGIN PUBLIC KEY')) {
    throw new Error('Bari installer metadata does not contain a valid release public key.');
  }
...
function normalizeTimeout(value: number | undefined): number {
  ...
  if (!Number.isSafeInteger(value) || value < 100 || value > 60_000) {
    throw new Error('Bari update timeout must be an integer between 100 and 60000 milliseconds.');
  }
  return value;
}
...
async function defaultFetchBytes(
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`Bari release server returned HTTP ${response.status} for ${url}`);
    }
...
async function defaultValidateInstalledVersion(prefix: string, version: string): Promise<void> {
  ...
  if (output.trim() !== version) {
    throw new Error(`Installed Bari version mismatch: expected ${version}, got ${output.trim()}`);
  }
}
...
function verifyArtifact(bytes: Buffer, manifest: McodeReleaseManifestV1): void {
  if (bytes.length !== manifest.installArtifact.size) {
    throw new Error(
      `Bari artifact checksum/size verification failed: expected ` +
        `${manifest.installArtifact.size} bytes, got ${bytes.length}.`,
    );
  }
  const digest = createHash('sha256').update(bytes).digest('hex');
  if (digest !== manifest.installArtifact.sha256) {
    throw new Error('Bari artifact checksum verification failed.');
  }
}
