...
export function acquireMcodeVersionedPrefixUpdateLock(activePrefix: string): () => void {
  ...
  try {
    ...
    if (blocked) throw new Error('Another Bari update is already running.');
  } catch (error) {
...
function readExpectedMetadata(
  ...
  if (metadata.version !== activation.expectedVersion) {
    throw new Error(
      `Bari release contains ${metadata.version}; expected ${activation.expectedVersion}.`,
    );
  }
  return metadata;
}
...
function assertVersionedRelease(
  ...
  for (const [file, contents] of expected) {
    if (readFileSync(file, 'utf8') !== contents) {
      throw new Error(`Bari release launcher is invalid: ${file}`);
    }
  }
}
...
function versionedReleaseLauncherContents(
  ...
  if (!existsSync(runtimeExecutable)) {
    throw new Error('Bari versioned launcher runtime is missing.');
  }
  for (const command of versionedCommands(metadata)) {
    if (!command.binEntry) {
      throw new Error(`Bari versioned ${command.name} package entry is missing.`);
    }
    ...
    if (
      ...
      throw new Error(`Bari versioned ${command.name} launcher path is invalid.`);
    }
...
function versionedRootLauncherContents(
  ...
  for (const command of versionedCommands()) {
    launchers.set(
      ...
      `#!/bin/sh\nset -eu\nroot=$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd -P)\nrelease=$(tr -d "\\r\\n" < "$root/current")\ncase "$release" in ""|*[!0-9A-Za-z._-]*) echo "Invalid Bari current release pointer." >&2; exit 1;; esac\nexec "$root/releases/$release/${command.releaseLauncher}" "$@"\n`,
    );
  }
  return launchers;
}
