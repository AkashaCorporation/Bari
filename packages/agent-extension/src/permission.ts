 ...
 * `@bari/agent-runtime` extension SPI's `pi.on('before_tool_call')` short-circuit event (design
 * document §4.3).
 *
 * Boundary policy: `@bari/permission` is a pure decision engine. `checkPermission(toolName, input,
...
import type { AgentExtension, BeforeToolCallHandler, ExtensionAPI } from '@bari/agent-runtime';
import type { PermissionDecision } from '@bari/permission';
