...
only receive short-lived Access Token leases. This package depends on `@bari/oauth-lease-protocol`;
it must not depend on Electron, `@bari/oauth-core` or a credential store.
...
系统代理发现和 Node fetch 初始化由新版 mcode-tools CLI 自身承担。宿主只提供认证 Broker 和资源分发，
