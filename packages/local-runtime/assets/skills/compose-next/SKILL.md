---
name: compose-next
description: Use for multi-step feature work, bug fixes, or refactors where requirements need to settle, one feature document should carry design + tasks + delivery evidence, and the change deserves independent review before merge. Use it only when the user explicitly requests this workflow — with `/compose-next`, by name, or in clear natural language. Never infer the request from task complexity alone. Not for one-shot edits, single-file tweaks, or answering questions; those need no orchestration overhead.
---
...
Enter this workflow only on an explicit user request. Any clear natural-language instruction to use this workflow counts just like `/compose-next`; the user does not need to name the skill. If the user has not clearly requested the workflow, do the work directly and run none of the phases below — do not infer consent because the task is large or resembles a Compose task.
...
- **Requirements clear and the feature deserves a durable document** → Workspace then Spec.

User gates and project overrides:

- If the user asks for `without worktree` or names a worktree/workspace path, use that choice and skip the default worktree gate. Do not ask again for worktree consent.
- If the user says `without spec`, "no spec needed", "this is a small fix", or equivalent, skip the durable feature document and its spec gate. Keep verification and review when the task still warrants them.
- A project instruction, `AGENTS.md`, or user-provided agent/worktree configuration may define a project-specific worktree path, branch convention, spec path, or spec format. Use that configuration instead of the defaults here, and record the override in the feature document or final report when it changes the normal artifact location.
...
Use the `ask_user` tool for every user decision:

- Put known choices in the options. Each option gets a concise label and a description explaining the consequence. List the recommendation first and mark it `(Recommended)`.
- For consequential choices, include 2–3 viable alternatives.
- When choices cannot be enumerated, still call `ask_user` for free text.
...
If `ask_user` is unavailable or the host cannot prompt for input (for example a headless run), resolve **this one decision** yourself and continue:
...
3. If the decision includes destructive or irreversible work, choose the non-destructive path that preserves progress; never auto-approve the destructive option.
4. State the option selected and the reason in the response.

This fallback applies to the current decision only; at every later decision point, try `ask_user` again.
...
- When targeting the worktree with a command, pass its absolute path as the working directory; omitted working directory uses the current session directory.
- Install dependencies per repository instructions, preferring lockfile-frozen modes over commands that mutate the lockfile. Confirm the toolchain is usable before continuing.
...
Before implementation, fix ambiguous requirements, contradictions, unresolved references, and unverifiable acceptance criteria. If the user is available, request document approval with `ask_user`; otherwise continue.
...
Use the feature document as the source of requirements, or the conversation for an undocumented mechanical change. When a feature document exists, set its `status: in-progress` on the first implementation commit. Execute tasks in dependency order. Track multi-step work with `todowrite`.
...
After implementation is verified and before finalizing the feature document, dispatch one fresh subagent (`worker` or a verifier-flavored custom Agent) to review the complete change.
...
If the user asks to finish but the path is unclear, use `ask_user` to settle:
